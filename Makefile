...
# Stack gate — the project's compile + test + assemble step. `./gradlew build`
# is the superset of `test`, so this is the single Gradle invocation that both
# CI (check.yml) and the pre-commit hook run; there is no separate build job.
stack-check: ## Compile, test and assemble the Kotlin project (./gradlew build)
	@./gradlew build
